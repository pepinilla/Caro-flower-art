...
- Title EN + ES: short, elegant (max 60 chars each)
- Excerpt EN + ES: 1 sentence (max 140 chars each)
- Blog post EN + ES: 400–500 words each
...
`.trim();
  ...
  const call = async () => {
    const resp = await openai.chat.completions.create({
      ...
      max_tokens: 2400,
      messages: [
        { role: "system", content: "Write natural, elegant brand copy in both English and Spanish. No emojis." },
  ...
  const gallery = imageUrls
    .map((u) => `
      <figure class="post-figure">
        <img src="${u}" alt="${escapeHtml(category)} handmade paper flowers" loading="lazy" decoding="async"/>
      </figure>`)
    .join("\n");

  // EN paragraphs wrapped in .en spans
  const enParagraphs = escapeHtml(copy.blogEn)
    .split("\n")
    .filter(Boolean)
    .map((p) => `<p class="post-p en">${p}</p>`)
    .join("");

  // ES paragraphs wrapped in .es spans
  const esParagraphs = escapeHtml(copy.blogEs)
    .split("\n")
    .filter(Boolean)
    .map((p) => `<p class="post-p es">${p}</p>`)
    .join("");
...
<head>
  ...
  <link rel="stylesheet" href="/site.css?v=20260305c" />
  <style>
...
<body>
  ...
  <main class="post-wrap">
    <div class="post-top">
      <a class="post-back" href="/blog/">← <span class="en">Back to Blog</span><span class="es">Volver al Blog</span></a>
    </div>

    <h1 style="margin:0;text-align:center">
      <span class="en">${escapeHtml(copy.titleEn)}</span>
      <span class="es">${escapeHtml(copy.titleEs || copy.titleEn)}</span>
    </h1>
  ...
  <script src="/site.js?v=20260305c"></script>

  <a href="https://wa.me/12368189725?text=Hi%20Caro!%20I%20saw%20your%20blog%20and%20loved%20it%20%F0%9F%8C%B8%20I%27d%20love%20to%20get%20a%20quote." target="_blank" rel="noopener" style="position:fixed;bottom:24px;right:24px;background:#25d366;color:#fff;border-radius:999px;padding:13px 22px;font-size:14px;font-weight:600;text-decoration:none;display:flex;align-items:center;gap:8px;box-shadow:0 6px 20px rgba(37,211,102,.4);z-index:1000;">💬 <span class="en">Get a quote</span><span class="es">Cotización</span></a>
</body>
</html>`;
}
...
async function pushBlogTrackingToSupabase({ slug, category, copy, imageUrls, imageKeys, postUrl, runKey, status, errorText }) {
  if (!supabase) { console.log("Supabase not configured."); return; }
  const contentHash = crypto.createHash("sha1").update([slug, copy?.titleEn, copy?.titleEs, ...(imageKeys || [])].join("|")).digest("hex");
  const row = {
    status: status || "published", slug, category,
    title_en: copy?.titleEn || null, title_es: copy?.titleEs || null,
    excerpt_en: copy?.excerptEn || null, excerpt_es: copy?.excerptEs || null,
    post_url: postUrl, image_urls: imageUrls || [], image_keys: imageKeys || [],
    content_hash: contentHash, run_key: runKey || null, ai_model: AI_MODEL, error: errorText || null,
  };
  ...
  for (const platform of platforms) {
    try {
      await supabase.from("social_posts").insert({
        ...
        image_urls: imageUrls, run_key: runKey,
      });
    } catch(e) { console.warn(`Could not save social post for ${platform}:`, e.message); }
  }
}
...
async function run() {
  ...
  try {
    ...
    ensureRobotsTxt();
    await pushBlogTrackingToSupabase({ slug, category, copy, imageUrls, imageKeys, postUrl, runKey, status: "published", errorText: null });
    ...
    try {
      await pushBlogTrackingToSupabase({ slug, category, copy: copy || { titleEn: null, titleEs: null, excerptEn: null, excerptEs: null }, imageUrls, imageKeys, postUrl, runKey, status: "failed", errorText: errText.slice(0, 900) });
    } catch (_) {}
